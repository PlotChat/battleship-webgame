...
export function ArenaBoard({parent = null, arenaController = null, variant = null, variantTrigger = () => {}}){
    if(arenaController == null) throw new Error("Arena Controller must be selected to be created as an ArenaBoard");
    ...
    for(let i = 0; i < arena.grid.length; i++){
        const currentBlock = new ArenaBlock({parent: parent, block: arena.grid[i], variant: variant, variantTrigger : variantTrigger});
        arenaBoard.appendChild(currentBlock);
    }
